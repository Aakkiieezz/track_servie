...
public interface UserServieDataRepository extends JpaRepository<UserServieData, UserServieDataKey>
{
    ...
    @Query(value = "SELECT new servie.track_servie.entity.vault.VaultUserServie(s.tmdbId, s.title) FROM Servie AS s"+" JOIN UserServieData AS usd ON s = usd.servie"+" WHERE usd.user = :user AND s.childtype = 'movie'")
    List<VaultUserServie> getUserMovies(@Param("user") User user);

    @Query(value = "SELECT new servie.track_servie.entity.vault.VaultUserServie(s.tmdbId, s.title) FROM Servie AS s"+" JOIN UserServieData AS usd ON s = usd.servie"+" WHERE usd.user = :user AND s.childtype = 'tv'")
    ...
    @Query(value = "SELECT usd FROM UserServieData AS usd WHERE usd.servie.tmdbId IN :tmdbIds AND usd.servie.childtype = :childtype AND usd.user = :user")
