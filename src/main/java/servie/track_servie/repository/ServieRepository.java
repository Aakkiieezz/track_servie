...
public interface ServieRepository extends JpaRepository<Servie, ServieKey>
{
    ...
    // @Query(value = "SELECT new servie.track_servie.payload.dtos.operationsHomePageDtos.ServieDtoHomePage(s.imdbId, s.tmdbId, s.childtype, s.title, s.posterPath, m.releaseDate, t.numberOfEpisodes, t.firstAirDate, t.lastAirDate, usd.episodesWatched, usd.completed) FROM Servie AS s "+"LEFT JOIN Movie AS m ON s.childtype = m.childtype AND s.tmdbId = m.tmdbId "+"LEFT JOIN Series AS t ON s.childtype = t.childtype AND s.tmdbId = t.tmdbId "+"JOIN UserServieData AS usd ON s = usd.servie "+"WHERE usd.userId = :userId AND s.childtype = :type")
    ...
    // +"          )"
    // +"     )")
    // Page<ServieDtoHomePage> findByAllFilters(@Param("user") User user,
    ...
    Page<ServieDtoHomePage> getServiesByHomePageFilter(@Param("user") User user, @Param("type") String type, @Param("watched") Boolean watched, @Param("languages") List<String> languages, @Param("statuses") List<String> statuses, @Param("startYear") Integer startYear, @Param("endYear") Integer endYear, Pageable pageable);

    @Query(value = "SELECT new servie.track_servie.payload.dtos.operationsServiePageDtos.ServieDtoServiePage(s.tmdbId, s.childtype, s.title, s.status, s.tagline, s.overview, s.backdropPath, m.releaseDate, m.runtime, t.numberOfSeasons, t.numberOfEpisodes, usd.episodesWatched, usd.completed) FROM Servie AS s"+" LEFT JOIN Movie AS m ON s.childtype = m.childtype AND s.tmdbId = m.tmdbId"+" LEFT JOIN Series AS t ON s.childtype = t.childtype AND s.tmdbId = t.tmdbId"+" JOIN UserServieData AS usd ON s = usd.servie"+" WHERE usd.user = :user AND s.childtype = :childtype AND s.tmdbId = :tmdbId")
    ServieDtoServiePage getServieForServiePage(@Param("user") User user, @Param("childtype") String childtype, @Param("tmdbId") Integer tmdbId);
