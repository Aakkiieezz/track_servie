...
public interface UserEpisodeDataRepository extends JpaRepository<UserEpisodeData, UserEpisodeDataKey>
{
        ...
        // @Query(value = "SEELCT new servie.track_servie.entity.UserEpisodeData(UserSeasonData userSeasonData, Integer episodeNo, Boolean watched)"
        //         +" FROM episode ep")
        // List<UserEpisodeData> getToggledEpisodes(Integer userId, Integer tmdbId, Integer seasonNo, Boolean watched);
        ...
        @Query(value = "SELECT new servie.track_servie.payload.dtos.operationsSeasonPageDtos.EpisodeDtoSeasonPage(episode.name, episode.episodeNo, episode.stillPath, episode.overview, episode.runtime, ued.watched)"
                        +" FROM Episode AS episode"
                        +" LEFT JOIN UserEpisodeData AS ued"
                        +"   ON ued.episodeNo = episode.episodeNo"
                        +"     AND ued.userSeasonData.seasonNo = episode.seasonNo"
